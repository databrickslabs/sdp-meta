...
- Every pipeline's configured `bronze.group` / `silver.group` is present in the onboarding file
- Every pipeline has the dataflowspec-table configuration required by its layer
- Every pipeline is referenced exactly once by the `pipelines` job
- Split silver tasks depend on their matching bronze tasks
- The onboarding `layer` covers the union of layers required by all pipelines
...
For the topology initially generated by `bundle-init`, choose the mode during
scaffolding. To add another topology—possibly using a different mode—use
`bundle-add-pipeline`. Existing pipeline resources can also be edited directly
before validation and redeployment.
